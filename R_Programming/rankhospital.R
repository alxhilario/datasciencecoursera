rankhospital <- function(state, outcome, num = "best") {
    ...
    na_removed_data <- ordered_data[!is.na(ordered_data[,2]),]
    
    num[which(toupper(num) == "BEST")] <- 1
    num[which(toupper(num) == "WORST")] <- nrow(na_removed_data)
    return(na_removed_data[as.integer(num), 1])
